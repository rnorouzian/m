...
mu <- sapply(1:length(fit), function(i) fit[[i]]$summary["mean","mu"])
lo <- sapply(1:length(fit), function(i) fit[[i]]$summary["95% lower","mu"])
hi <- sapply(1:length(fit), function(i) fit[[i]]$summary["95% upper","mu"])

x <- 0:(L-1)

plot(x, mu, type = "l", ylim = range(lo, hi), ylab = "Effect Size (dint)", lwd = 4, lty = 2, lend = 1,
     xaxt = "n", xlab = "Time", panel.l = axis(1, at = x, labels = c(if(!is.na(mu[1]))
       "Post_1" else NULL, if(!is.na(mu[2]) & is.null(LO)) "Post_2" else if(!is.na(mu[2]) & !is.null(LO)) "Long" else NULL,
...
points(x, mu, pch = 22, cex = 5, bg = "cyan", col = "magenta", xpd = NA)

text(x, c(lo, mu, hi), 
     round(c(lo, mu, hi), 3),  cex = .7, font = 2, xpd = NA)
   }

for(i in 1:L) G(m[[i]], main = if(is.null(main)) n[[i]] else main[i])
