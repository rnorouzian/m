                             ...
                             zero = 0, FE = FALSE, legend = FE, shrink = FALSE, ...)
{
  ...
  if(shrink) points(x$theta[5,], -x$sigma, pch=21, , col="gray50", bg= adjustcolor("gray40", .5), cex=1.2)
...
d.prepos <- function(d = NA, study.name = NA, group.name = NA, n = NA, mpre = NA, mpos = NA, sdpre = NA, sdpos = NA, r = NA, rev.sign = FALSE, autoreg = FALSE, t = NA, sdif = NA, sdp = NA, F1 = NA, df2 = NA, post, control, outcome, ...) 
{
...
plot(x, mu, type = "l", ylim = range(lo, hi), ylab = "Effect Size (dint)", lwd = 4, lty = 2, lend = 1,
     xaxt = "n", xlab = "Time", panel.l = axis(1, at = x, labels = c(if(!is.na(mu[1]) & is.null(LO))
